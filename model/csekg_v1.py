...
class Aggregator(nn.Module):
    ...
    def forward(
        ...
        neigh_relation_emb = (
            # 1] 加了 edge_imp
            entity_emb[tail] * edge_relation_emb * edge_imp.view(-1, 1)
        )  # [-1, embedding_size]
...
class Ours_v1(KnowledgeRecommender):
    ...
    def __init__(self, config, dataset):
        super(Ours_v1, self).__init__(config, dataset)
        ...
        self.edge_index, self.edge_type, edge_imp = self.get_edges(self.kg_graph)
    ...
    def get_edges(self, graph):
        ...
        # imp = torch.ones(type.shape)
        node_imp = self.get_node_imp(graph)
        edge_soft_imp = self.imp_modelling(graph, node_imp)
        return index.to(self.device), type.to(self.device), edge_soft_imp.to(self.device)

    def get_node_imp(self, graph):
        ...
        for i,ps in item_dict.items():
            # property_cnt += Counter(filter(lambda x: x>=n_items, ps))
            for p in ps:
                if p >= n_items:
                    property_cnt[p] += 1
...
""" DGL方案
https://docs.dgl.ai/en/latest/generated/dgl.nn.functional.edge_softmax.html
"""
# from dgl.nn.functional import edge_softmax
# import dgl
# def softmax(src, dst, edata):
#     g = dgl.graph(src, dst)
#     return edge_softmax(g, edata, norm_by='src')

""" PyG 方案
https://pytorch-geometric.readthedocs.io/en/latest/_modules/torch_geometric/utils/softmax.html
"""
# def softmax(src, index, dim=0):
#     # src: Tensor,
